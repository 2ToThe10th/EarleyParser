...
bool EarleyParser::CheckWord(std::string &str) {
  std::vector<std::vector<Position>> D(str.size() + 1,
...
void EarleyParser::Scan(std::vector<std::vector<Position>> &D,
                        int current_position, std::string str) {
...
void EarleyParser::Predict(std::vector<std::vector<Position>> &D,
                           int current_position, bool &is_changed) {
  std::list<Position> to_add;
  for (auto &i : D[current_position]) {
    for (auto &j : rules_) {
      if (i.position < i.rule->to.size() && j.from == i.rule->to[i.position]) {
...
void EarleyParser::Complete(std::vector<std::vector<Position>> &D,
                            int current_position, bool &is_changed) {
  std::list<Position> to_add;
    ...
    if (small_rule.position == small_rule.rule->to.size()) {
      for (auto &full_rule : D[small_rule.start]) {
        if (full_rule.position < full_rule.rule->to.size() &&
            full_rule.rule->to[full_rule.position] == small_rule.rule->from) {
